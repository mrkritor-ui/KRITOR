(function(){
  ...
  function applyLevel(n){
    ...
    if(window.KritorTileImage)window.KritorTileImage.applySizes(grid);
    saveState();
  }
  ...
  function makeTile(item,index,eagerLimit){
    ...
    /* Same responsive renditions and blur placeholder as the catalogue, so the
       store's first paint costs no more than the archive's. */
    a.appendChild(window.KritorTileImage
      ?window.KritorTileImage.build(first,{alt:item.title||"",eager:index<eagerLimit})
      :(function(){const i=document.createElement("img");i.src=assetPath(first);i.alt=item.title||"";i.loading="lazy";i.decoding="async";return i})());
    return a;
  }
  ...
  function render(){
    ...
    applyLevel(level);
    const eagerLimit=window.KritorTileImage?window.KritorTileImage.eagerCount(grid):4;
    items.forEach((item,index)=>grid.appendChild(makeTile(item,index,eagerLimit)));
    applyLevel(level);
